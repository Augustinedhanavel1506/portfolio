...
// ── Skill badge textures, generated on canvas so we don't ship third-party logo assets ──
function createSkillTexture(label: string, bg: string): THREE.CanvasTexture {
  const s = 256;
  ...
  const fs = label.length > 4 ? 42 : label.length > 2 ? 52 : 80;
...
const skillDefs = [
  ...
  { label: "JEE", bg: "#5382A1" },
  { label: "Hibernate", bg: "#8B5E34" },
  { label: "JDBC", bg: "#4B6FB0" },
  { label: "HTML", bg: "#E44D26" },
  { label: "CSS", bg: "#264DE4" },
  { label: "JS", bg: "#C9A200" },
  { label: "PHP", bg: "#777BB4" },
  { label: "SQL", bg: "#00758F" },
];
const allTextures = skillDefs.map((s) => createSkillTexture(s.label, s.bg));
